		...
		const mergedData = enrollments.map((e) => {
			...
			return {
				...
				last_accessed_at: prog?.last_accessed_at
					? new Date(prog.last_accessed_at).getTime()
					: 0,
			};
		});

		const ongoingCourses = mergedData.filter(
...
function ProgressVisualization({ percentage }: { percentage: number }) {
	...
	const data = [{ name: "Progress", value: 100, fill: BRAND_PRIMARY }];
