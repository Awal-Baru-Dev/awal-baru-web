...
import { enrollmentKeys, useEnrolledCourseIds } from "@/features/enrollments";
import {
	getDokuJsUrl,
	useCreatePayment,
	useVerifyPayment,
...
function CoursesPage() {
	...
	const {
		...
		error: allError,
		refetch: refetchAll,
	} = useCourses({ enabled: !hasSearchQuery });
	const {
		data: searchResults,
	...
	const { data: enrolledCourseIds, refetch: refetchEnrollments } =
						...
						if (result.success) {
							if (result.status === "paid") {
								toast.success("Pembayaran Berhasil!", {
									description: "Selamat! Semua kursus sudah dapat diakses.",
								});
								refetchEnrollments();
				...
				<section className="py-8">
					<div className="container mx-auto max-w-6xl">
							...
							<div>
								...
								{regularCourses && regularCourses.length > 0 ? (
									<p className="text-muted-foreground mt-1">
										{regularCourses.length} kursus{" "}
										{hasSearchQuery ? "ditemukan" : "tersedia"}
										{!hasSearchQuery &&
											bundle &&
											" - beli satuan atau hemat dengan paket lengkap"}
									</p>
						...
						{isLoading ? (
							...
							<CourseGridError
								message={
									error instanceof Error ? error.message : "Gagal memuat kursus"
								}
								onRetry={() => refetch()}
							/>
						) : (
